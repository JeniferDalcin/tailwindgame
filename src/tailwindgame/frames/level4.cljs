...
(defn Frame []
  (let [class (rf/subscribe [::subs/?class])
    ...
    [:div.flex.w-full.min-h-screen
     ...
     ;; div left side
     [:div
      {:class "flex flex-col bg-slate-950 w-1/2"} 
      
      ;; div with bg color
      [:div
       {:class "flex flex-col px-10 py-5 gap-7"}
       ...
       [:div.gap-3.text-slate-300.text-lg
        ...
        " for controlling how flex and grid items are positioned along a container's cross axis."]
       ...
       [:div.flex.flex-1.flex-col.gap-3
        ...
        [:span.text-white.text-lg.bg-slate-800.px-2.rounded.self-start.ml-6 "items-baseline"]
        [:span.text-white.text-lg.bg-slate-800.px-2.rounded.self-start.ml-6 "items-stretch"]]
      ...
      [:div
       ...
       [:div.relative.h-full.w-full.bg-slate-950.rounded-md.border.border-indigo-700
        [:img.absolute.h-20.w-20
         {:class "inset-y-1/2 -translate-y-1/2 left-7"
          :src "images/earth.png"}]
        
        ;; astronaut 
        [:div.flex.flex-1.w-full.h-full.py-10.pl-10
