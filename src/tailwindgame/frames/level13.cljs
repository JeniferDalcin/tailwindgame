(ns tailwindgame.frames.level13
  (:require
   [re-frame.core :as rf]
     ...
     [:div
      ...
      [:div
       ...
       [:div.gap-3.text-slate-300.text-lg
        ...
        "for controlling the direction of flex items and for controlling how flex and grid items
         are positioned along a container's cross axis."]
       ...
       [gui/CodeEditor
        {:value @class
         ...
         :pre-code "flex"
         :rows 7
         :desired-class "flex-col-reverse items-center"
         :on-click-check (fn [] 
     ...
     [:div
      ...
      [:div
       ...
       [:div.relative.h-full.w-full.bg-slate-950.rounded-md.border.border-indigo-700.p-7
        [:img.absolute.bottom-7.h-20.w-20
         {:class "inset-x-1/2 -translate-x-1/2"
